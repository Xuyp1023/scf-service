...
public class ScfOrderRelationService extends BaseService<ScfOrderRelationMapper, ScfOrderRelation> {
     ...
     * 订单关联关系保存
     */
    public List<ScfOrderRelation> addOrderRelation(String anEnterType, Long anEnterId, String anInfoType,
            String anInfoIdList) {
        ...
        // 订单管理关系保存
        if (StringUtils.equals(ScfOrderRelationType.ORDER.getCode(), anEnterType)) {
            for (String anInfoId : anInfoIds) {
                ...
                // 检查关系是否存在，若存在则不再添加
                if (checkRelationExists(anOrderRelation.getOrderId(), anOrderRelation.getInfoId(),
                        anOrderRelation.getInfoType())) {
                    continue;
                }
        ...
        // 当两个id都不为订单时
            ...
            if (Collections3.isEmpty(orderRelationList)) {
                // 通过票据生成订单，并于此票据建立
                if (StringUtils.equals(anEnterType, ScfOrderRelationType.ACCEPTBILL.getCode())) {
                ...
                // 通过应收账款生成
                if (StringUtils.equals(anEnterType, ScfOrderRelationType.RECEIVABLE.getCode())) {
    ...
    private boolean checkRelationExists(Long anOrderId, Long anInfoId, String anInfoType) {
        Map<String, Object> queryMap = QueryTermBuilder.newInstance().put("orderId", anOrderId).put("infoId", anInfoId)
                .put("infoType", anInfoType).build();
