...
public class ScfRequestProtacalService extends BaseService<ScfRequestProtacalMapper, ScfRequestProtacal> {
     ...
     * @return
     */
    public boolean updateProtacalInfo(final ScfRequestProtacal protacal, final BigDecimal anBalance) {
        protacal.setAgreeName("保兑仓业务三方合作协议");
        final ScfRequestProtacal tmpProtacal = this.selectByPrimaryKey(protacal.getRequestNo());
        if (tmpProtacal == null) {
        ...
        elecAgreementService.addElecAgreementInfo(elecAgreement, "threePartProtocol",
                Arrays.asList(protacal.getThreeNo(), Long.parseLong(elecAgreement.getFactorNo())));
        return true;
    }
