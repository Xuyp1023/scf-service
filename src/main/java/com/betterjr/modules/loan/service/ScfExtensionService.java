...
public class ScfExtensionService extends BaseService<ScfExtensionMapper, ScfExtension> {
    ...
    public BigDecimal getLoanBalance(String anRequestNo, String anStartDate) {
        ...
        // 总额
        Map<String, BigDecimal> feeMap = payPlanService.getOverDueFee(tomorrowDate, anStartDate,
                payPlan.getShouldPrincipalBalance(), payPlan.getFactorNo().toString());
    ...
    public Map<String, Object> payAssigned(String anRequestNo, String anPayDate, BigDecimal payBalance) {
        ...
        if (overDueDays > 0 && overDueDays > param.getGraceDays()) {
            Map<String, BigDecimal> feeMap = payPlanService.getOverDueFee(tomorrowDate, anPayDate,
                    payPlan.getSurplusPrincipalBalance(), custNo);
        ...
        if (overDueDays > 0 && overDueDays < param.getGraceDays()) {
            overManagementBalance = payPlanService.getFee(payPlan.getFactorNo(), payPlan.getSurplusPrincipalBalance(),
                    payPlan.getManagementRatio(), payPlan.getStartDate(), payPlan.getPlanDate());
            overInterestBalance = payPlanService.getFee(payPlan.getFactorNo(), payPlan.getSurplusPrincipalBalance(),
                    payPlan.getRatio(), payPlan.getStartDate(), payPlan.getPlanDate());
        }
    ...
    public Map<String, Object> getInterest(BigDecimal ratio, BigDecimal managementRatio, BigDecimal extensionBalance) {
        ...
        BigDecimal managementBalance = MathExtend.multiply(extensionBalance, managementRatio)
    ...
    public ScfExtension addExtension(ScfExtension anExtension) {
        ...
        payPlanService.fillPayPlan(record, plan);
