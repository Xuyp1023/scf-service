...
public class ScfOfferService extends BaseService<ScfOfferMapper, ScfOffer> {
    ...
    public ScfOffer addOffer(ScfOffer anOffer) {
        ...
        if (StringUtils.equals("1", enquiry.getEnquiryMethod())) {
        ...
        ScfEnquiryObject object = scfEnquiryObjectService.findByEnquiryNoAndObject(anOffer.getEnquiryNo(),
                anOffer.getFactorNo());
    ...
    public ScfOffer saveModifyOffer(ScfOffer anOffer, Long anId) {
        ...
        if (StringUtils.equals("0", anOffer.getBusinStatus())) {
            enquiryService.saveUpdateOfferCount(anOffer.getEnquiryNo(), -1);
            ScfEnquiryObject object = scfEnquiryObjectService.findByEnquiryNoAndObject(anOffer.getEnquiryNo(),
                    anOffer.getFactorNo());
    ...
    public Page<ScfOffer> queryOfferList(Map<String, Object> anMap, int anFlag, int anPageNum, int anPageSize) {
        if (anMap.get("businStatus") == null || StringUtils.isEmpty(anMap.get("businStatus").toString())) {
    ...
    public List<ScfEnquiryObject> queryOfferByEnquiryObject(String enquriyNo) {
        ...
        Map<String, Object> qyObjectMap = QueryTermBuilder.newInstance().put("enquiryNo", enquiry.getEnquiryNo())
                .build();
        List<ScfEnquiryObject> list = null;
        if (StringUtils.equals("2", enquiry.getEnquiryMethod())) {
            // 主动报价
    ...
    public ScfOffer findOfferDetail(Long anFactorNo, String anEnquiryNo) {
        Map<String, Object> qyMap = QueryTermBuilder.newInstance().put("factorNo", anFactorNo)
                .put("enquiryNo", anEnquiryNo).put("businStatus", new String[] { "1", "3" }).build();
