...
public interface CommissionDailyStatementMapper extends Mapper<CommissionDailyStatement> {
    ...
    // * @return
    // */
    // @Select("select SUM(F_TOTAL_BALANCE) from t_cps_daily_statement where D_PAY_DATE>=#{startDate} and
    // D_PAY_DATE<=#{endDate} and L_OWN_CUSTNO=#{ownCustNo} and C_BUSIN_STATUS in (#{businStatus});")
