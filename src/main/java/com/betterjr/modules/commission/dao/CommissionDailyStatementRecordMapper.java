...
public interface CommissionDailyStatementRecordMapper extends Mapper<CommissionDailyStatementRecord> {
    ...
    @Insert("INSERT INTO t_cps_daily_statement_record(ID,L_DAILY_STATEMENT_ID,C_DAILY_STATEMENT_REFNO,L_PAY_RECORD_ID,C_PAY_RECORD_REFNO,L_RECORD_ID,C_RECORD_REFNO,"
            ...
            + "SELECT ID,#{dailyStatement.id},#{dailyStatement.refNo},ID,C_REFNO,L_RECORD_ID,C_RECORD_REFNO,D_IMPORT_DATE,T_IMPORT_TIME,C_PAYNO,D_PAY_DATE,T_PAY_TIME"
            + ",F_PAY_BALANCE,C_PAY_TARGET_BANK,C_PAY_TARGET_BANKNAME,C_PAY_TARGET_BANK_FULLNAME,C_PAY_TARGET_BANK_ACCO,C_PAY_TARGET_BANK_ACCO_NAME,C_PAY_BANK,"
            + "C_PAY_BANK_NAME,C_PAY_BANK_FULLNAME,C_PAY_BANK_ACCO,C_PAY_BANK_ACCO_NAME,C_PAY_RESULT,C_PAY_COMMENT,#{dailyStatement.businStatus},"
            + "#{dailyStatement.businStatus},#{dailyStatement.ownCustNo},#{dailyStatement.ownCustName},,#{dailyStatement.operOrg},#{dailyStatement.regOperId},"
            + "#{dailyStatement.regOperName},#{dailyStatement.regDate},#{dailyStatement.regTime} FROM t_cps_pay_result_record"
            + " WHERE d_pay_date=#{dailyStatement.payDate} and l_custno=#{dailyStatement.ownCustNo}")
