...
public class CommissionFileService extends BaseService<CommissionFileMapper, CommissionFile> {
    ...
    private List<Map<String, Object>> resolveFileToListMap(InputStream anIs, List<CustFileCloumn> anFileCloumnList, CommissionFile anFile)
        ...
        while (rows.hasNext()) {
             ...
             * StringUtils.isBlank(ExcelUtils.getStringCellValue(currentRow.getCell(2)))){ break; }
             * 
             * }
             */
    ...
    /**
     * 检查当前企业的解析记录是否审核完全，如果已经审核完成，不允许再次上传文件解析 true 表明今天还可以上传文件 false 表明今天已经不可以上传文件解析
     * 
     * @param anCustNo
     * @param anImportDate
     * @return
    ...
    public boolean checkFilePermitOperator(Long anCustNo, String anImportDate) {
        ...
        Map queryMap = QueryTermBuilder.newInstance().put("custNo", anCustNo).put("importDate", anImportDate)
                .put("businStatus", CommissionConstantCollentions.COMMISSION_BUSIN_STATUS_AUDIT).build();
        List<CommissionFile> fileList = this.selectByProperty(queryMap);
    ...
    private boolean checkFilePermitOperator(Long anFileId) {
        ...
        Map queryMap = QueryTermBuilder.newInstance().put("fileId", anFileId).build();
        List<CommissionFile> fileList = this.selectByProperty(queryMap);
        if (Collections3.isEmpty(fileList)) {
            return true;
        }
    ...
    public List<CommissionFile> queryFileListByFileDownId(Long anFileId) {
        ...
        Map queryMap = QueryTermBuilder.newInstance().put("fileDownId", anFileId).build();
