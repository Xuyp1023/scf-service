...
public class CommissionFileDownService extends BaseService<CommissionFileDownMapper,CommissionFileDown>{
     ...
     */
        ...
        private void checkFileDownStatus(CommissionFileDown anFileDown) {
            ...
            checkStatus(anFileDown.getConfirmStatus(), CommissionConstantCollentions.COMMISSION_FILE_CONFIRM_STATUS_EFFECTIVE, true, "当前文件记录已经已经审核通过，不能重复审核");
            checkStatus(anFileDown.getConfirmStatus(), CommissionConstantCollentions.COMMISSION_FILE_CONFIRM_STATUS_INEFFECTIVE, true, "当前文件记录已经审核未通过，不能重复审核");
